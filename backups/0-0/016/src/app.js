...
canvas.addEventListener('mousedown', (event) => {
    ...
    if (drawingModeMenu.value === 'click') {
        ...
        if (selectedLine) {
        ...
        } else if (snapToEndpointEnabled && highlightedEndpoint) {
            // Use the highlighted endpoint as the start or end point
            if (!isDrawing) {
                startPoint = highlightedEndpoint;
                isDrawing = true;
            } else {
                const endPoint = highlightedEndpoint;
                lines.push({ start: startPoint, end: endPoint });
                isDrawing = false;
                startPoint = null;
                drawCanvas(); // Redraw the canvas
            }
        } else if (snapToMidpointEnabled && highlightedMidpoint) {
            // Use the highlighted midpoint as the start or end point
            if (!isDrawing) {
                startPoint = highlightedMidpoint;
                isDrawing = true;
            } else {
                const endPoint = highlightedMidpoint;
                lines.push({ start: startPoint, end: endPoint });
                isDrawing = false;
                startPoint = null;
                drawCanvas(); // Redraw the canvas
            ...
            // Default behavior without snapping
            if (!isDrawing) {
                startPoint = mousePos;
                isDrawing = true;
            } else {
...
canvas.addEventListener('mousemove', (event) => {
    ...
    if (snapToEndpointEnabled) {
        // Calculate and highlight the nearest endpoint
        highlightedEndpoint = getNearestEndpoint(mousePos.x, mousePos.y);
        drawCanvas(); // Redraw the canvas
        if (highlightedEndpoint) {
            highlightEndpoint(highlightedEndpoint); // Highlight the endpoint
        }
    } else if (snapToMidpointEnabled) {
        // Calculate and highlight the nearest midpoint
        highlightedMidpoint = getNearestMidpoint(mousePos.x, mousePos.y);
        drawCanvas(); // Redraw the canvas
        if (highlightedMidpoint) {
            highlightMidpoint(highlightedMidpoint); // Highlight the midpoint
        }
        ...
        highlightedMidpoint = null; // Clear the highlighted midpoint
        highlightedEndpoint = null; // Clear the highlighted endpoint
        drawCanvas(); // Redraw the canvas
    }
